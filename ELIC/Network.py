...
from models.local_global_net import BottleneckBlock
# from utils.global_local_block import Global_local_module
...
class TestModel(CompressionModel):
    ...
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        ...
        #         ResidualBottleneckBlock(N),
        #         ResidualBottleneckBlock(N),
        #         # BottleneckBlock(ch_in=N, arch=self.arch),
        #         conv(N, M),
        #         # BottleneckBlock(ch_in=M, arch=self.arch),
        #         AttentionBlock(M),
        #     )
        #
        #     self.g_s = nn.Sequential(
        ...
        #         deconv(N, N * 3 // 2),
        #         nn.ReLU(inplace=True),
        #         BottleneckBlock(ch_in=N * 3 // 2, arch=self.arch),
        #         conv3x3(N * 3 // 2, 2 * M),
        #     )
    ...
    def forward(self, x, rate_map=None, noisequant=False):
        y = self.g_a(x)
