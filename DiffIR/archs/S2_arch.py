...
class denoise(nn.Module):
    def __init__(self,n_feats = 64, n_denoise_res = 5,timesteps=5, group=8):
    ...
    def forward(self,x, t,c):
        ...
        t =t/self.max_period
        t=t.view(-1,1)
        t = t.unsqueeze(2).repeat(1, self.group, 1)
        # print(" x shape", x.shape)
        # print(" t shape", t.shape)
        # print(" c shape", c.shape)
        c = torch.cat([c,t,x],dim=2)
        # print(" new c shape", c.shape)
        fea = self.resmlp(c)
...
class DiffIRS2(nn.Module):
    def __init__(self,         
        ...
        self.condition = CPEN(n_feats=64, n_encoder_res=n_encoder_res, group=group)

        self.denoise= denoise(n_feats=64, n_denoise_res=n_denoise_res,timesteps=timesteps, group=group)

        self.diffusion = DDPM(denoise=self.denoise, condition=self.condition ,n_feats=64,linear_start= linear_start,
  linear_end= linear_end, timesteps = timesteps, group=group)
