...
from DiffIR.archs.VGG16 import VGGPerceptual
...
class latent_encoder_lrelu(nn.Module):
    ...
    def __init__(self, in_chans=6, embed_dim=64, block_num=4, stage=1, group=4, patch_expansion=0.5, channel_expansion=4):
        ...
        self.ResBlock = nn.Sequential(nn.Conv2d(in_chans * 16, embed_dim * 2, kernel_size=3, stride=2),  ## Downsampling  32*32
                                      ResidualBottleneckBlock(in_ch=embed_dim*2),
                                      ResidualBottleneckBlock(in_ch=embed_dim*2),
                                      # AttentionBlock(embed_dim * 2),

                                      nn.Conv2d(embed_dim*2, embed_dim * 4, kernel_size=3, stride=2),  ## Downsampling  ## 16*16
                                      ResidualBottleneckBlock(in_ch=embed_dim * 4),
                                      ResidualBottleneckBlock(in_ch=embed_dim * 4),
                                      AttentionBlock(embed_dim * 4),
                                      )

        assert self.group <= 16  ##
        self.pool = nn.AdaptiveAvgPool2d((group, group))
    ...
    def forward(self, inp_img, gt=None):
        if gt is not None:
            ...
            x0 = self.pixel_unshuffle(inp_img)
            x = torch.cat([x0, gt0], dim=1)
        else:
            x = inp_img
            x = self.pixel_unshuffle(x)

        x = self.ResBlock(x)#.squeeze(-1).squeeze(-1)
        x= self.pool(x)
        x = rearrange(x, 'b c h w-> b (h w) c')
        S1_IPR = []
        x = self.mlp(x)
        S1_IPR.append(x)
        # print("----- x end  shape", x.size())
...
##  使用双线性池化
# class latent_encoder_bilinear(nn.Module):
#
#     def __init__(self, in_chans=6, embed_dim=64, block_num=4, stage=1, group=4, patch_expansion=0.5,
#                  channel_expansion=4):
#         super(latent_encoder_bilinear, self).__init__()
#
#         assert in_chans == int(6 // stage), "in chanel size is wrong"
#
#         self.group = group
#         self.local_output_dim = group ** 2
#         self.global_output_dim = 4 * embed_dim
#
#         self.pixel_unshuffle = nn.PixelUnshuffle(4)
#
#         if stage == 1:
...
#             nn.Conv2d(3 * 16, embed_dim, 3, 1, 1),
#             nn.GELU(),
#         )
#
#         self.blocks = nn.ModuleList()
...
#             block = nn.Sequential(
#                 ResidualBottleneckBlock(embed_dim),)
...
#             nn.LeakyReLU(0.1, True),
#             nn.Conv2d(embed_dim * 2, self.local_output_dim, kernel_size=3, padding=1),
#             nn.LeakyReLU(0.1, True),
#             AttentionBlock(self.local_output_dim),
#         )
#         self.pool = nn.AdaptiveAvgPool2d((group, group))
#
#
#         self.vgg16 = VGGPerceptual()
#         self.conv3 = nn.Sequential(
#             nn.Conv2d(512, self.global_output_dim, 3, 1, 1),
#             nn.GELU(),
#         )
#
#
...
#     def forward(self, inp_img, gt=None):
#         if gt is not None:
#             x01 = self.conv_01(inp_img)
#             x02 = self.conv_02(gt)
#             xIn = x01 + x02
#         else:
#             xIn = inp_img
#
#         x_local = self.pixel_unshuffle(xIn)
#         x_local= self.conv1(x_local)
#         for block in self.blocks:
#             x_local = block(x_local) + x_local
#         x_local = self.conv2(x_local)
#         x_local = self.pool(x_local)
#
#         x_deep = self.vgg16(xIn)
#         x_deep = self.conv3(x_deep)
#
#         if self.group < 16:  #   assume imaeg size is bigger 256
#             x_deep = self.pool(x_deep)
#
#         # print(" x_local shape", x_local.shape)
#         # print(" x_depp shape", x_deep.shape)
#
#         #  双线性池化
#         N = x_local.size()[0]
#         local_feature_H = x_local.size()[2]
#         local_feature_W = x_local.size()[3]
#
#         global_feature_H = x_deep.size()[2]
#         global_feature_W = x_deep.size()[3]
#
#         if (local_feature_H != global_feature_H) | (local_feature_W != global_feature_W):
#             print("convert size")
#             x_deep = F.interpolate(x_deep, size=(local_feature_H, local_feature_W), mode='bilinear')
#
#         x_local = x_local.view(N, self.local_output_dim, local_feature_H * local_feature_W)
#         x_global = x_deep.view(N, self.global_output_dim, global_feature_H * global_feature_W)
#
#         x_out = torch.bmm(x_global, torch.transpose(x_local, 1, 2)) / (
#                 local_feature_H * local_feature_W)  # Bilinear
#         # assert X.size() == (N, D, D)
#         x_out = torch.reshape(x_out, (N, self.local_output_dim * self.global_output_dim))
#         x_out = torch.sign(x_out) * torch.sqrt(torch.abs(x_out) + 1e-5)  # Signed-sqrt normalization
#         x_out = torch.nn.functional.normalize(x_out, dim=1)  # L2 normalization
#         x_out = torch.reshape(x_out, (N, self.local_output_dim, self.global_output_dim))
#
#         S1_IPR = []
#         x = self.mlp(x_out)
...
class DiffIR_mlkkS1(nn.Module):
    def __init__(self,
        ...
        super(DiffIR_mlkkS1, self).__init__()
        ...
        # self.E = latent_encoder_lrelu(in_chans=6, embed_dim=embed_dim, block_num=n_encoder_res, stage=1, group=group, patch_expansion=0.5, channel_expansion=4)
        # self.E = latent_encoder_bilinear(in_chans=6, embed_dim=embed_dim, block_num=n_encoder_res, stage=1, group=group, patch_expansion=0.5, channel_expansion=4)
        self.E = CPEN_mlkk(n_feats=64, n_encoder_res=n_encoder_res, group=group)
