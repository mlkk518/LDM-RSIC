...
class Upsample_HIM(nn.Module):
    def __init__(self, n_feat):
        super(Upsample_HIM, self).__init__()
...
class HIM(nn.Module):
    def __init__(self, dim, num_heads, bias, embed_dim, group =4, k_dim=128, v_dim=128):
        ...
        self.v_dim = v_dim
        self.group = group
        num_blocks = 3  ## Group = 4， 则需要进行3次上采样，才能达到  32*32
        ...
        if group != 1:
            self.UpSampling = nn.ModuleList([Upsample_HIM(embed_dim*4) for i in range(num_blocks)])
...
#
## !!! 做过实验  将上下采样换成 DWT，结果较差，可能原因，网络不擅长学习这种数据
...
class BasicLayer(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, embed_dim, num_blocks, group):
        ...
        # # build blocks
        # self.blocks_local = nn.ModuleList([ResidualBottleneckBlock(dim) for i in range(num_blocks)])

        # # build blocks
        self.blocks_Global = nn.ModuleList(
...
class DiffIRS1(nn.Module):
    def __init__(self,
        ...
        self.E = CPEN(n_feats=64, n_encoder_res=n_encoder_res, group=group)
